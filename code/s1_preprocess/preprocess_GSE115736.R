...
dat_count <- file.reader(GSE115736_path, gene_file, feature_index = 1, column_index=5, header=TRUE, unit="count")
dat_tpm <- file.reader(GSE115736_path, gene_file, feature_index = 1, column_index=6, header=TRUE, unit="tpm")
dat_fpkm <- file.reader(GSE115736_path, gene_file, feature_index = 1, column_index=7, header=TRUE, unit="fpkm")
...
# dat_count_p3 <- dat.filter(dat_count_p2,5,10) # 16113 genes remained
# dat_tpm_p3 <- dat.filter(dat_tpm_p2,5,1) # 21852 genes remained 
# dat_fpkm_p3 <- dat.filter(dat_fpkm_p2,5,1) # 19760 genes remained 
# dat_cpm_p3 <- dat.filter(dat_cpm_p2,5,1) #18343 genes remained 
# read meta data 
meta <- read.table("./Raw/GSE115736/GSE115736_meta.txt",header=TRUE,sep="\t")
# store all processed data into a list 
# GSE115736_dat <- list(meta = meta ,count = dat_count_p3,tpm = dat_tpm_p3,fpkm = dat_fpkm_p3,cpm = dat_cpm_p3)
GSE115736_dat_raw <- list(meta = meta ,count = dat_count_p2,tpm = dat_tpm_p2,fpkm = dat_fpkm_p2,cpm = dat_cpm_p2)
# save all preprocessed data and meta data for downstream analysis 
save(GSE115736_dat_raw,file="./Data/GSE115736_dat_raw.RData")