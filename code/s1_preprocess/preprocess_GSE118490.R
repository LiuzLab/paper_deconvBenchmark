# preprocess GSE118490
source("./R/Functions_all.R")
GSE118490_path = "./Raw/GSE118490/RSEM_quant/"
# read expr from .txt file (output of RSEM)
# unit: count, tpm and fpkm 
gene_file <- list.files(path=GSE118490_path,pattern="genes.results")
dat_count <- file.reader(GSE118490_path, gene_file, feature_index = 1, column_index=5, header=TRUE, unit="count")
dat_tpm <- file.reader(GSE118490_path, gene_file, feature_index = 1, column_index=6, header=TRUE, unit="tpm")
dat_fpkm <- file.reader(GSE118490_path, gene_file, feature_index = 1, column_index=7, header=TRUE, unit="fpkm")
...
# dat_count_p3 <- dat.filter(dat_count_p2,1,10) # 14594 genes remained
# dat_tpm_p3 <- dat.filter(dat_tpm_p2,1,1) # 12814 genes remained 
# dat_fpkm_p3 <- dat.filter(dat_fpkm_p2,1,1) # 12512 genes remained 
# dat_cpm_p3 <- dat.filter(dat_cpm_p2,1,1) #12857 genes remained 
# read meta data 
meta <- read.table("./Raw/GSE118490/GSE118490_meta.txt",header=TRUE,sep=",")
# store all processed data into a list 
GSE118490_dat_raw <- list(meta = meta ,count = dat_count_p2,tpm = dat_tpm_p2,fpkm = dat_fpkm_p2,cpm = dat_cpm_p2)
# save all preprocessed data and meta data for downstream analysis 
save(GSE118490_dat_raw,file="./Data/GSE118490_dat_raw.RData")