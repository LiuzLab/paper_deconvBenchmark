...
gene_file <- list.files(path=GSE64655_path,pattern="genes.results")
dat_count <- file.reader(GSE64655_path, gene_file, feature_index = 1, column_index=5, header=TRUE, unit="count")
dat_tpm <- file.reader(GSE64655_path, gene_file, feature_index = 1, column_index=6, header=TRUE, unit="tpm")
dat_fpkm <- file.reader(GSE64655_path, gene_file, feature_index = 1, column_index=7, header=TRUE, unit="fpkm")
...
dat_count_p3 <- dat.filter(dat_count_p2,5,10) # 18663 genes remained
dat_tpm_p3 <- dat.filter(dat_tpm_p2,5,1) # 19246 genes remained 
dat_fpkm_p3 <- dat.filter(dat_fpkm_p2,5,1) # 17822 genes remained 
dat_cpm_p3 <- dat.filter(dat_cpm_p2,5,1) #17115 genes remained 
# read meta data 
meta <- read.table("./Raw/GSE64655/GSE64655_meta.txt",header=TRUE,sep="\t")
# store all processed data into a list 
GSE64655_dat <- list(meta = meta ,count = dat_count_p3,tpm = dat_tpm_p3,fpkm = dat_fpkm_p3,cpm = dat_cpm_p3)
GSE64655_dat_raw <- list(meta = meta ,count = dat_count_p2,tpm = dat_tpm_p2,fpkm = dat_fpkm_p2,cpm = dat_cpm_p2)
# save all preprocessed data and meta data for downstream analysis 
save(GSE64655_dat, file="./Data/GSE64655_dat.RData")
save(GSE64655_dat_raw, file="./Data/GSE64655_dat_raw.RData")
