...
for(u in 1:length(unit)){
    ...
    for(m in 1:length(methods)){
      write_path <- paste0("/mnt/data/haijing/simDeconv/paper_deconvBenchmark/output/Sim1/libSize/", methods[m])
      # Step 1: write mix
...
for(u in 1:length(unit)){
  for(s in 1:length(sim_model)){
    ...
    for( m in 1:length(methods)){
      read_path <- paste0("/mnt/data/haijing/simDeconv/paper_deconvBenchmark/output/Sim1/libSize/", methods[m],"/out/")
...
# save results 
# save results 
save.image(file = "./output/Sim1/sim1_libSize_deconvolution_crossPlatform.RData")
save(list = ls(pattern = "estW"), file = "./output/Sim1/libSize/sim1_libSize_estW.RData")