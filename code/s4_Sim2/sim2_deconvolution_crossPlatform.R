...
load("./output/sim2_ref.RData")
load("./output/sim2_estW.RData")
...
# library(Rserve)
# Rserve(args="--no-save")
methods <- "MMAD" # Please run the method one by one (CIBERSORTx, CIBERSORT and MMAD) to avoid exaustion of computing resource. 
for(u in 1:length(unit)){
  for(s in 1:length(mix_type)){
    ...
    for(m in 1:length(methods)){
      write_path <- paste0("/abs_path/output/Sim2/", methods[m]) # Please set the path to the targeted directory. 
      # Step 1: write mix
...
for(u in 1:length(unit)){
  for(s in 1:length(mix_type)){
    ...
    for( m in 1:length(methods)){
      read_path <- paste0("/abs_path/output/Sim2/", methods[m],"/out/") # Please set the path to the targeted directory. 
...
# save results 
save(list = ls(pattern = "estW"), file = "./output/Sim2/sim2_estW.RData") # Please set the path to the targeted directory. 